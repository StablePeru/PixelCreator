#!/bin/bash
# ============================================================================
# Forest Archer — PixelCreator Showcase #2
# 48x48 · 8 frames · idle(4) + walk(4) · 16-color earth/forest palette
# Pipeline: project:init → palette:create → canvas:create → frames → tags →
#           animation:create-tag → export:gif → export:spritesheet
# ============================================================================
set -euo pipefail
...
echo "=== Forest Archer Showcase Pipeline ==="
...
rm -rf "$P"
pnpm pxc project:init -n forest-archer --path . -o silent
...
echo "[5/8] Drawing 8 frames via draw:batch + stdin..."
for i in $(seq 1 4); do
  FNUM=$(printf "%03d" "$i")
...
mkdir -p "$P/exports/forest-archer"
pnpm pxc export:gif -c "$C" --dest "$P/exports/forest-archer/archer-idle.gif" --tag idle --loop 0 -p "$P" -o silent
pnpm pxc export:gif -c "$C" --dest "$P/exports/forest-archer/archer-walk.gif" --tag walk --loop 0 -p "$P" -o silent

# ── 8. Export spritesheet ────────────────────────────────────────────────────
echo "[8/8] Exporting spritesheet..."
pnpm pxc export:spritesheet -c "$C" --dest "$P/exports/forest-archer/archer-spritesheet.png" --layout grid --columns 4 -p "$P" -o silent

echo ""
echo "=== Pipeline complete ==="
echo "Exports in: $P/exports/forest-archer/"
ls -la "$P/exports/forest-archer/" 2>/dev/null || echo "(no exports dir yet)"
