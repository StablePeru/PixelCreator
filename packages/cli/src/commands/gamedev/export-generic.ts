...
export default class GamedevExportGeneric extends BaseCommand {
  static description = 'Export canvas as engine-agnostic spritesheet with JSON metadata';
  ...
  async run(): Promise<void> {
    ...
    const { flags } = await this.parse(GamedevExportGeneric);
    ...
    formatOutput(format, cmdResult, (data) => {
      this.log(`Exported "${data.canvas}" for generic engine (scale ${data.scale}x)`);
