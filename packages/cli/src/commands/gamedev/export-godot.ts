...
export default class GamedevExportGodot extends BaseCommand {
  static description = 'Export canvas as Godot-ready spritesheet, .tres SpriteFrames, and .tscn scene';
  ...
  async run(): Promise<void> {
    ...
    const { flags } = await this.parse(GamedevExportGodot);
    ...
    formatOutput(format, cmdResult, (data) => {
      this.log(`Exported "${data.canvas}" for Godot (scale ${data.scale}x)`);
