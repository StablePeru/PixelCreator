...
export default class BrushShow extends BaseCommand {
  ...
  async run(): Promise<void> {
    ...
    formatOutput(format, result, (r) => {
      ...
      this.log(
        `  Size: ${r.size}px | Shape: ${r.shape} | Spacing: ${r.spacing} | Opacity: ${r.opacity}`,
      );
      this.log(`  Pixel Perfect: ${r.pixelPerfect}`);
      if (r.pressureSensitivity) {
        const ps = r.pressureSensitivity;
        this.log(
          `  Pressure: ${ps.enabled ? 'enabled' : 'disabled'} | Curve: ${ps.curve} | Min Size: ${ps.minSize} | Min Opacity: ${ps.minOpacity}`,
        );
      }
