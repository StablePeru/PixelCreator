...
export function createRectTool(cb: ToolCallbacks): DrawTool {
  let active = false;
  ...
  return {
    ...
    async onEnd() {
      ...
      if (rw < 1 || rh < 1) return;
      await cb.sendDraw('rect', {
        canvas, x: rx, y: ry, width: rw, height: rh,
    ...
    getPreview(): PreviewShape | null {
      ...
      const rx = Math.min(x1, x2), ry = Math.min(y1, y2);
      return {
        type: 'rect', color: cb.getColor(), fill: cb.getFillMode(),
