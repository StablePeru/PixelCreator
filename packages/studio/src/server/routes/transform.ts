...
export const transformRoutes = new Hono<{ Variables: { projectPath: string; historyStack: HistoryStack } }>();
...
function applyTransform(
  c: any,
  operation: string,
  transform: (projectPath: string, canvasName: string, layerId: string, frameId: string, historyStack: HistoryStack) => void,
) {
...
transformRoutes.post('/transform/flip', async (c) => {
  ...
  const { canvas: canvasName, direction, layer, frame } = body as {
    canvas: string; direction: 'h' | 'v'; layer?: string; frame?: string;
  };
  ...
  historyStack.push({ operation: `flip-${direction}`, canvasName, layerId, frameId, beforeBuffer: before });
...
transformRoutes.post('/transform/rotate', async (c) => {
  ...
  const historyStack = c.get('historyStack');
  const body = await c.req.json();
  const { canvas: canvasName, turns, layer, frame } = body as {
    canvas: string; turns: 1 | 2 | 3; layer?: string; frame?: string;
  };
...
transformRoutes.post('/transform/brightness', async (c) => {
  ...
  const historyStack = c.get('historyStack');
  const body = await c.req.json();
  const { canvas: canvasName, amount, layer, frame } = body as {
    canvas: string; amount: number; layer?: string; frame?: string;
  };
  ...
  writeLayerFrame(projectPath, canvasName, layerId, frameId, result);
  historyStack.push({ operation: 'brightness', canvasName, layerId, frameId, beforeBuffer: before });
...
transformRoutes.post('/transform/desaturate', async (c) => {
  ...
  writeLayerFrame(projectPath, canvasName, layerId, frameId, result);
  historyStack.push({ operation: 'desaturate', canvasName, layerId, frameId, beforeBuffer: before });
...
transformRoutes.post('/transform/hue-shift', async (c) => {
  ...
  const historyStack = c.get('historyStack');
  const body = await c.req.json();
  const { canvas: canvasName, degrees, layer, frame } = body as {
    canvas: string; degrees: number; layer?: string; frame?: string;
  };
