...
- **Core engine** — new `packages/core/src/core/terrain-blend-engine.ts` (pure primitive) with `generateBlendMasks`, `composeBlendedTile`, and `buildTransitionTileset`. Produces the 47 blob-47 transition tiles between two biomas, reusing `BLOB_47_CONFIGS` from `autotile-engine` (now exported). Dither rendering goes through an ordered 4×4 Bayer matrix for deterministic pixel-art edges; `strength` ∈ [0, 1] controls the transition depth. `includeInverse` appends the reversed 47 tiles (target-over-source).
- **Core slice** — new `packages/core/src/core/asset-biome-blend-engine.ts` with `validateBiomeBlendAssetSpec`, `buildBiomeBlendAsset`, and `scaffoldBiomeBlendAssetSpec`. Validation covers source/target canvas existence, tileSize divisibility on both canvases, `tileSizeMultipleOf`, and a combined-histogram `maxColors` check (with an actionable `palette:generate` hint that points at both biomas). Build slices the top-left tile from each canvas, calls `buildTransitionTileset`, composes an atlas PNG via `composeTilesetImage`, and emits either a Godot 4 `TileSet` `.tres` (explicit `col:row/0 = 0` per cell) or a `{name}.blend.json` metadata manifest; the spec is re-embedded as `{name}.asset.json` for reproducibility.
- **CLI** — `pxc asset:init` gains `--type biome-blend` plus `--source-canvas`, `--target-canvas`, `--tile-size WxH` (required), `--blend-mode dither`, `--strength <0..1>`, and `--include-inverse`. `asset:list` gains a `biome-blend` display branch (`source=X target=Y tile=WxH blend=dither(0.5)`). `asset:build` watch-mode now watches both source and target canvases when the spec is biome-blend. `asset:validate` and `asset:build` need no slice-specific changes — they already dispatch through the core entry points. Total CLI commands still 246.
- **Tests** — new `packages/core/test/core/terrain-blend-engine.test.ts` (15 tests: mask count, dimensions, binary-alpha, determinism, fully-connected / isolated configs, strength boundaries, `composeBlendedTile` purity, `buildTransitionTileset` inverse). New `packages/core/test/core/asset-biome-blend-engine.test.ts` (20 tests: schema parsing, validation paths, build outputs for both engines, dispatch entry points, scaffold). New E2E suite `packages/cli/test/e2e/asset-biome-blend-pipeline.test.ts` drives `asset:init --type biome-blend → asset:list → asset:validate → asset:build` end-to-end and asserts atlas + Godot `.tres` + inverse doubling. Suite total: 1778 → 1837 green.
- **Showcase** — new `showcase/asset-biome-blend-demo/pipeline.sh` that creates two 16×16 bioma canvases (grass, sand), runs `asset:init --type biome-blend --tile-size 16x16 --blend-mode dither --strength 0.5 --engine godot`, validates, and builds into `exports/grass_to_sand/` (atlas PNG + Godot 4 TileSet `.tres` + asset.json).
